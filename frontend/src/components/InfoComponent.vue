<script setup lang="ts">
    ...
    onMounted(() => {
        hoverEl.value
        .parentNode.removeChild(hoverEl.value);
    ...
    function adaptHover(keepALive = false) {
        ...
        const position = infoEl.value
        .getBoundingClientRect();
        const offset = 30;
        hoverEl.value
        .style.top = `${Math.min(position.y, window.innerHeight - hoverEl.value.offsetHeight - offset)}px`;
        hoverEl.value
        .style.left = `${Math.max(position.x - hoverEl.value.offsetWidth, offset)}px`;
...
<template>
    <div v-show="!empty" class="info" @mouseenter="toggleHover()" @mouseout="toggleHover()" ref="infoEl">
        <i class="info-icon"></i>
