        ...
        .then((results: IResults) => {
            ...
            if(tabsEl.value.getIndex() === 1) {
                ...
                setTimeout(() => {
                    ...
                    x.domain(data.map(function (d: any): string { return util.label(d) }));
                    ...
                    .enter().append("rect")
                    .attr("class", "bar")
                    .attr("x", function (d: any): number { return x(util.label(d)) as number })
                    .attr("width", x.bandwidth())
                    .attr("y", function (d: any): number { return y(util.score(d)) })
                    .attr("height", function (d: any) {return height - y(d.score); })
                    .attr("fill", "var(--color-primary)")
                    .append("title").text((d:any) => util.label(d)+": "+util.score(d).toString());
                    svg.append("g")
