<script setup lang="ts">
    ...
    function getNodeChildren(entity: IEntity): HTMLElement[] {
        return svgEl.value
        .querySelector(`#${getNodeId(entity)}`)
    ...
    function createTree(data: any, { // data is either tabular (array of objects) or hierarchy (nested objects)
        ...
        const root: d3.HierarchyNode<NodeData> = (id != null || parentId != null)
...
<template>
    <div class="results-graph" @mousedown="mouseEnableSVGInteraction">
        <div class="results-graph-selection" :class="hasPreselectedEntity ? 'active' : ''">
            ...
            <div>
                <ButtonComponent small class="selection-button--select" @click="selectPreselected()">Select</ButtonComponent>
...
<style scoped lang="scss">
    ...
    .results-graph {
        ...
        &-wrapper {
            ...
            &::before {
                background: linear-gradient(0deg, var(--color-bg-light) 0%, transparent $offset, transparent calc(100% - $offset), var(--color-bg-light) 100%);
            }
            &::after {
                background: linear-gradient(90deg, var(--color-bg-light) 0%, transparent $offset, transparent calc(100% - $offset), var(--color-bg-light) 100%);
            }
