...
- [Node.js](https://nodejs.org/en) (ships with *NPM* by default)
- [Node Pavckage Manager (NPM)](https://www.npmjs.com/)
...
6. **IF** the previous step terminated with success, commit changes.

5. Merge issue branch to test branch and check functionality again (4., 5.):
...
A *Sonar* analysis can be run using the [sonar-scanner](https://docs.sonarqube.org/display/SCAN/Analyzing+with+SonarQube+Scanner), or by using the related *Maven* plugin. Given this as a prerequisite, run:
...
## Integration from Third-party Applications
...
The *HTML* file at `src/main/examples/webpage/index.htm` gives a complete example on how to integrate Antelope via an `iframe` element. The example reads a textfield and sends it to the annotation service for entity mapping. Then, the annotation service will display the results in a select component within the `iframe`. When the user selects a result within the iframe, a message is send to the parent application. The message contains the selected entity data and is processed within the function `onMessage()`.

> See the related schema graph at `src/main/examples/webpage/integrationConcept.svg`.

### Via `RESTful AP`

More specific integrations with the annotation service can be achieved thorugh the provided *REST* compliant API. THe API documentation is available on `https://service.tib.eu/annotation/v3/api-docs/openapi`.
