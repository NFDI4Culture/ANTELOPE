...
export class GraphTidytreeComponent implements OnInit {
  ...
  createTree(data : any, { // data is either tabular (array of objects) or hierarchy (nested objects)
      ...
      haloWidth = 5, // padding around the labelssuper
    } ): void {
      ...
      // Compute labels and titles. (call given label function for each node)
      const descendants = root.descendants();
      const L:string[]|null = descendants.map((d:any) => label(d));

      // create a copy of the children array for each node (for store tree collapsing info)
      descendants.forEach((d:any) => {
        d.childrenTemp = d.children;
      });
      ...
      function update(source:any) {
        ...
        const textNode = node.append("text") 
            ...
            .text((d:any, i:any) => d.data.label/*L[i]*/ )
